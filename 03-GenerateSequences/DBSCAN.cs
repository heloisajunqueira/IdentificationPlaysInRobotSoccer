...
namespace ClusteringSpace
{
    public class DBSCAN
    {
		...
		public static int[] auxStates;

		public static int nameState=0;

		public  static List<Path> Sequence = new List<Path> ();

		static FrechetDistance frechet;

		public  static int[,] arrayState;

		public static bool isFirstState = true ; 
        ...
        public static void DoDBSCAN(List<Path> paths, float eps, int minPathsForCluster)
		{
			...
			{	
				...
				foreach(PathCollection c in clusters)
				{
					...
					clusterDist.Add(c.getCenterDistPath());
				}
				
				epsInt=System.Convert.ToInt16(eps*10);
				string directoryr = Directory.GetCurrentDirectory();
				string filenamer = "/data/";
				string filer = "statistics_Helios_"+minPathsForCluster.ToString()+"_"+epsInt.ToString()+".txt";
				...
				string file = "resultCluster_Helios_"+minPathsForCluster.ToString()+"_"+epsInt.ToString()+".txt";
		...
		public static void setDistMetric(int distMetric_) 
		{
		...
		public static int verifyState(int[] auxStates,int auxTotalCluster, int totalPath, int totalLines)
        {
			int auxCountComposition = 0;
			int auxEquivalent = 0;

			int[] CompositionCluster = new int[auxTotalCluster]; // composition List do array que está entrando
			int[] CompositionClusterState = new int[auxTotalCluster]; // composition list do array auxiliar dos estados já esxistentes

			for(int i=0; i<auxTotalCluster;i++){ // Inicializar composition list
				CompositionCluster[i]=0;
			}

			for(int i=0; i<auxTotalCluster;i++){ // Inicilaizar compositiom list
				CompositionClusterState[i]=0;
			}
			
			if(isFirstState == true){
				arrayState = new int[(totalLines/12)*151,13];
				nameState=0;
				arrayState[0,0] = nameState; //armazena estado 0
				for (int j=0; j < 12 ;j++){
					arrayState[0,j+1] = auxStates[j];
				...
				for(int i=0;i<12;i++)
				{
					for(int j=0;j<auxTotalCluster;j++){
						if(auxStates[i] == j){ // montar composition list do array que está entrando
							CompositionCluster[j]++;
						}
					}
				}

				for(int i=0;i<nameState;i++) //linha
				{
					for(int j=1;j<13;j++){  //coluna
					
						for(int k=0;k<auxTotalCluster;k++){ // montar compositionClusterState de cada estado já encontrado
							if(arrayState[i,j] == k){
								CompositionClusterState[k]++;
							}
						}
					}
					for(int t=0; t<auxTotalCluster; t++){// comparar compositions list
						//onsole.Write("nameState:{0},", nameState);
						if(CompositionCluster[t] != 0 || CompositionClusterState[t] != 0){
							if(CompositionCluster[t] == CompositionClusterState[t])
							{
								auxEquivalent = 1;
								auxCountComposition = auxCountComposition + CompositionCluster[t];
							}
						}
					} 
					//Console.WriteLine("nameState:{0}/auxCountComposition: {1} / auxEquivalent: {2} ", nameState, auxCountComposition, auxEquivalent);
					if(auxCountComposition >= 11 && auxEquivalent==1)
					{
						int positionArrayState = arrayState[i,0]; // armazena a variável de qual cluster é equivalente
						return (positionArrayState);
					}
					for(int z=0; z<auxTotalCluster;z++){ // Inicilaizar compositiom list
						CompositionClusterState[z]=0;
					}
					auxCountComposition = 0;
				}

				//Console.WriteLine("nameState:{0}", nameState);
				arrayState[nameState,0] = nameState;

				for (int j=0; j < 12 ;j++){ // for para armazenar o novo estado
					arrayState[nameState,j+1] = auxStates[j];
				}
				nameState++;								
			}
			return(nameState-1);
		...
		//pegar este primeiro estado da matriz, colocar como um elemento com índice(que será o estado), e comparar com os demais
		//se pegar cada linha desta comparação a sequencia já resultará na sequencia de estados.
		public static void FindState(int[,] auxSeq, int auxCol, bool canPrint, int auxTotalCluster, int totalPath, int totalLines) 
        {
			int[,] auxState = new int[auxCol,12];
			for(int i=0; i<12 ; i++){ //linha
				for(int j=0; j<auxCol ;j++){ //coluna
					auxState[j,i]= auxSeq[i,j];
				}
			}
			int node = 0;
			//Console.WriteLine("Find STATE");
			List<Node> nodeCluster = new List<Node> ();
			for(int i=0; i<auxCol ; i++){ //linha
				auxStates = new int[12];
				for(int j=0; j<12 ;j++){ //coluna
					auxStates[j] = auxState[i,j];
					//Console.Write("{0},", auxStates[j]);
				}
				node = verifyState(auxStates, auxTotalCluster, totalPath, totalLines); // chama para cada state
				//Console.WriteLine();
				nodeCluster.Add(new Node(node+1,0.0,0.0));
			}
			Sequence.Add (new Path (nodeCluster));


			if(canPrint == true){
				Console.WriteLine("______________FINAL_LIST_____________");
				string directoryData4 = Directory.GetCurrentDirectory();
				string filenameData4 = "/data/States.txt"; //fill with the data file
				string textFile4 = directoryData4 + filenameData4;
				using(StreamWriter writetext = new StreamWriter(textFile4))
				{
					for(int i=0;i<nameState;i++) //linha
					{
						writetext.Write("Estado[{0}]: ",arrayState[i,0]);
						Console.Write("Estado[{0}]: ",arrayState[i,0]);
						for(int j=0;j<12;j++){ // coluna
							writetext.Write("{0},", arrayState[i,j+1]);
							Console.Write("{0},", arrayState[i,j+1]);
						}
						writetext.WriteLine();
						Console.WriteLine();
					}
				...
				int countSequence = 0;
				string directoryData5 = Directory.GetCurrentDirectory();
				string filenameData5 = "/data/Sequences.txt"; //fill with the data file
				string textFile5 = directoryData5 + filenameData5;
				string val="0";
				int cons=0;
				using(StreamWriter writetext2 = new StreamWriter(textFile5))
				{
					
					foreach(Path sequence in Sequence)
					{
						writetext2.WriteLine(">Seq{0}",countSequence, CultureInfo.InvariantCulture);
						Console.WriteLine(">Seq{0}",countSequence, CultureInfo.InvariantCulture);
						writetext2.Write("{0},", sequence.ToString(), CultureInfo.InvariantCulture);
						Console.Write("{0},", sequence.ToString());

						if(sequence.points.Count()<149)  // adicionar padding
						{
							for(int i=sequence.points.Count()+1;i<150;i++){
								writetext2.Write("{0},", val, CultureInfo.InvariantCulture);
								Console.Write("{0},", val, CultureInfo.InvariantCulture);
								cons=i-sequence.points.Count()+1;
							}
							Console.WriteLine("\n Size: {0}, cons:{1}",sequence.points.Count(), cons); 
						}
						writetext2.WriteLine();
						Console.WriteLine();
						countSequence++;
					}
					//foreach(Path sequence in Sequence)  alterar primeiro estado
					//{
					//	writetext2.WriteLine(">Seq {0}",countSequence, CultureInfo.InvariantCulture);
					//	Console.WriteLine(">Seq {0}",countSequence, CultureInfo.InvariantCulture);
					//	writetext2.WriteLine("{0}", sequence.ToString(), CultureInfo.InvariantCulture);
					//	Console.WriteLine("{0}", sequence.ToString());
					//	countSequence++;
					//}
				} 
			}
        ...
        public static double FindDistance(Path path1_, Path path2_) 
        {
			...
			{
				Console.WriteLine("Invalid distansequences");
				/*if (saveDistances)
				{
