...
pair<Matrix, std::vector<Feature*> > GLMRegressor::selectFeatures(Matrix &t2,
        std::vector<Feature*> &f2) {
	std::cout << "GLM Regressor - Select Features" << std::endl;
	auto isNewBetter = [](double newV, double oldV) {
		return (oldV - newV > 0.000025) ? true : false; // Original
	};

    BestFirst<GLM> selector(t2, *lTrainTable, f2, GLM::regressorFactory,
			Evaluator::mse, isNewBetter, false, threadNum, minFeat,
			std::numeric_limits<double>::infinity());

    return std::make_pair(selector.transform(t2), selector.getFeatureList());
}
