...
Statistician<V>::Statistician(int histogramSizeIn, int kIn, const V *h1In,
		...
		const double *backgroundIn, const unsigned int *keyListIn) :
...
// Adjust count and calculate statistic
	...
	unsigned int maxSize = k* (histogramSize)+(k-1);
	//std::cout << "k : " << k << std::endl;
	//std::cout << "hist : " << (histogramSize-1) << std::endl;
	//std::cout << "key : " << keyList[maxSize] << std::endl;

	//double size = (histogramSize-1)*k+(k-1);
	//std::cout << "maxSize2: " << keyList[size] << std::endl;
	unsigned int d2 = 0.0;
	for (unsigned int i = 0; i < histogramSize; i++) {
		...
		for (unsigned int j = 0; j < k; j++) {
			unsigned int indice = i * k + j;
			//std::cout << "keyList : " << keyList[indice] << std::endl;
			e1 *= background[keyList[indice]];
			e2 *= background[keyList[indice]];
		}
		...
		if (!Util::isEqual(denom, 0.0)) { // false
			d2 += (a1 * a2) / denom;
			//std::cout << "NOT skipped a row " << std::endl;
		} //else { //true
			//std::cout << "demom:" << denom << std::endl;
			//std::cout << "Skipped a row d2sSimilarityHelper" << std::endl;
		//}
	}
...
double Statistician<V>::d2starSimilarity() {
	...
	for (unsigned int i = 0; i < histogramSize; i++) {
		...
		if (!Util::isEqual(e, 0.0)) {
			...
			//std::cout << "Skipped a row d2starSimilarity" << std::endl;
		//}
	}
	return d2;
}
